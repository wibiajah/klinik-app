<x-admin-layout title="Data Laboratorium">
    <style>
        :root {
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3 mb-0 page-title">Data Laboratorium</h1>
        ...
        <div class="row mb-4">
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stat-card">
                    ...
                    <div class="stat-number">{{ $statistics['siap_periksa'] }}</div>
                    <div class="stat-label">Menunggu Diperiksa</div>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stat-card">
        ...
        <!-- Tambahan stat card untuk Selesai -->
        ...
        <div class="custom-card">
            <div class="card-header py-3 d-flex justify-content-between align-items-center" style="background-color: var(--light-gray); border-bottom: 1px solid var(--border-gray);">
                <h6 class="m-0 fw-bold" style="color: #333;">
                    Daftar Laboratorium - {{ \Carbon\Carbon::parse($tanggal)->format('d F Y') }}
                </h6>
    ...
    <i class="fas fa-sort sort-icon"></i>
                        ...
                        <tbody>
                            @forelse($laboratorium as $key => $item)
                            <tr>
                                <td class="fw-semibold">{{ $laboratorium->firstItem() + $key }}</td>
                                <td>
                                    @if($item->no_antrian)
    ...
    <div class="d-flex align-items-center gap-2">
        ...
        @endif
                                ...
                                <td>{{ $item->nik }}</td>
                                <td class="fw-semibold">{{ $item->tgl_lahir->format('d/m/Y') }}</td>
                                <td>
                                    @if($item->created_at)
                                ...
                                <td>
                                    @if($item->status_pemeriksaan == 'menunggu')
                                        <span class="badge" style="background-color: #ffc107; color: #212529;">Menunggu Pemeriksaan</span>
                                    @elseif($item->status_pemeriksaan == 'sedang_diperiksa')
                                        <span class="badge" style="background-color: #17a2b8; color: white;">{{ $item->status_label }}</span>
                                    @else
    ...
    @if($item->selesai_periksa_by)
        <div class="fw-semibold text-success">{{ $item->userSelesaiPeriksa->name }}</div>
        <small class="text-muted">Selesai: {{ $item->selesai_periksa_at->format('d/m H:i') }}</small>
    @elseif($item->mulai_periksa_by)
        <div class="fw-semibold text-primary">{{ $item->userMulaiPeriksa->name }}</div>
        <small class="text-muted">Mulai: {{ $item->mulai_periksa_at->format('d/m H:i') }}</small>
    @elseif($item->set_antrian_by)
        <div class="fw-semibold text-warning">{{ $item->userSetAntrian->name }}</div>
        <small class="text-muted">Set Antrian: {{ $item->set_antrian_at->format('d/m H:i') }}</small>
    @else
        <span class="text-muted">-</span>
    @endif
</td>
    ...
    <div class="dropdown">
        ...
        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $item->id }}" style="min-width: 150px;">
            <!-- Detail Button -->
            <li>
             <a class="dropdown-item" href="{{ route('admin.laboratorium.detail', $item->id) }}">
    <i class="fas fa-eye me-2 text-primary"></i> Detail
    @if($item->set_antrian_by || $item->mulai_periksa_by || $item->selesai_periksa_by)
        <small class="text-muted d-block ms-4">Lihat riwayat petugas</small>
    @endif
</a>
            </li>
            
            @if($item->status_pemeriksaan == 'menunggu' && !$item->no_antrian)
                <li><hr class="dropdown-divider"></li>
                
                <!-- Set Antrian Button -->
                <li>
                    <form method="POST" action="{{ route('admin.laboratorium.set-antrian', $item->id) }}" 
                          class="d-inline w-100" onsubmit="return confirm('Set nomor antrian?')">
                        @csrf
            ...
            @elseif($item->status_pemeriksaan == 'menunggu' && $item->no_antrian)
                <li><hr class="dropdown-divider"></li>
                
                <!-- Mulai Pemeriksaan Button -->
                <li>
                    <form method="POST" action="{{ route('admin.laboratorium.update-status', $item->id) }}" 
                          class="d-inline w-100" onsubmit="return confirm('Mulai pemeriksaan laboratorium?')">
                        @csrf
                        <input type="hidden" name="status" value="sedang_diperiksa">
                        <button type="submit" class="dropdown-item">
                            <i class="fas fa-play-circle me-2 text-success"></i> Mulai Pemeriksaan
                        </button>
                    </form>
                </li>
            ...
            @endif
                            ...
                            <tr>
                                <td colspan="11">
                                    <div class="empty-state">
                                        ...
                                        <p class="mb-0">Tidak ada data laboratorium pada tanggal {{ \Carbon\Carbon::parse($tanggal)->format('d F Y') }}</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
                ...
                @if($laboratorium->hasPages())
                    <div class="d-flex justify-content-center p-3" style="border-top: 1px solid var(--border-gray);">
                        {{ $laboratorium->appends(['tanggal' => $tanggal])->links() }}
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="border-radius: 8px; border: none;">
                <div class="modal-header" style="background-color: var(--light-gray); border-bottom: 1px solid var(--border-gray);">
                    <h5 class="modal-title fw-bold text-dark">Input Hasil Laboratorium</h5>
                ...
                <form id="hasilForm" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Hasil Laboratorium *</label>
                            <textarea name="hasil_lab" class="form-control" rows="4" required 
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="catatan_lab" class="form-control" rows="3" 
    ...
    <script>
        ...
        function openHasilModal(id) {
            ...
            form.action = `/admin/laboratorium/${id}/konfirmasi`;
        ...
        function sortTable(columnIndex) {
            ...
            rows.sort((a, b) => {
                ...
                if (columnIndex === 0) {
                    // Regular numeric sorting for No column
                    aValue = parseInt(aValue) || 0;
                    bValue = parseInt(bValue) || 0;
                } else if (columnIndex === 1) {
                    // Special sorting for No. Antrian (extract number from LAB-XX format)
                    const extractAntrianNumber = (str) => {
                        if (str === '-') return 0;
                        const match = str.match(/LAB-(\d+)/);
                        return match ? parseInt(match[1]) : 0;
                    };
                    aValue = extractAntrianNumber(aValue);
                    bValue = extractAntrianNumber(bValue);
                } else if (columnIndex === 5 || columnIndex === 6) {
                    // Date sorting for Tanggal Lahir and Waktu Transfer
                    ...
                    aValue = extractDate(aValue);
                    bValue = extractDate(bValue);
                }
